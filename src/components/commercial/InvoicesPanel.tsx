...
export function InvoicesPanel({
  ...
  const visibleInvoices = fixedSiteId
    ? data.invoices.filter((invoice) => invoice.site_id === fixedSiteId)
    : fixedCustomerId
      ? data.invoices.filter((invoice) => invoice.customer_id === fixedCustomerId)
      : data.invoices;
  const invoicedOfferIds = new Set(
    data.invoices.map((invoice) => invoice.offer_id).filter(Boolean),
  );
  const eligibleOffers = data.offers.filter(
    (offer) =>
      !invoicedOfferIds.has(offer.id) &&
      !["rejected", "expired"].includes(offer.status) &&
      (!fixedCustomerId || offer.customer_id === fixedCustomerId) &&
      (!fixedSiteId || offer.site_id === fixedSiteId),
  );
  ...
  async function createInvoice(event: FormEvent<HTMLFormElement>) {
    ...
    const { data: offerItems, error: itemsReadError } = await supabase
      .from("offer_items")
      .select("*")
  ...
  return (
    <section className="rounded-2xl border border-border bg-card shadow-card">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4 sm:p-5">
        <div>
          <h2 className="font-display text-lg font-bold">Rechnungen & Zahlung</h2>
          <p className="text-sm text-muted-foreground">
            Angebot übernehmen, Fälligkeit überwachen und Zahlung verbuchen.
          </p>
        </div>
        {canWrite && (
          <Button
            ...
            disabled={!eligibleOffers.length}
            className="bg-brand text-white hover:bg-brand/90"
          >
            <Plus className="h-4 w-4" /> Rechnung aus Angebot
          </Button>
        )}
      </header>
        ...
        {visibleInvoices.length ? (
          <div className="space-y-3">
            {visibleInvoices.map((invoice) => {
              ...
              return (
                <article
                  key={invoice.id}
                  className={`grid gap-3 rounded-xl border p-4 md:grid-cols-[auto_minmax(0,1fr)_auto_auto] md:items-center ${overdue ? "border-amber-300 bg-amber-50/50" : "border-border"}`}
                >
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="truncate font-semibold">{invoice.subject}</h3>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {customerName(customersById.get(invoice.customer_id))} · fällig{" "}
                      {formatDate(invoice.due_date)}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold">{formatEur(invoice.gross_amount)}</p>
                    <p className="text-xs text-muted-foreground">
                      offen{" "}
                      {formatEur(
                        Math.max(0, Number(invoice.gross_amount) - Number(invoice.paid_amount)),
                      )}
                    </p>
                  </div>
                  {canWrite ? (
                    <Select
                      ...
                      onValueChange={(status) =>
                        updateStatus(invoice.id, status, Number(invoice.gross_amount))
                      }
                    >
          ...
          <div className="grid min-h-56 place-items-center rounded-xl border border-dashed border-border text-center">
            <div>
              ...
              <p className="mt-3 font-medium">Noch keine Rechnung</p>
              <p className="text-sm text-muted-foreground">
                {eligibleOffers.length
                  ? "Übernimm ein Angebot ohne erneute Eingabe."
                  : "Lege zuerst ein Angebot an."}
              </p>
            </div>
          </div>
        )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg">
          <form onSubmit={createInvoice}>
            <DialogHeader>
              <DialogTitle>Rechnung aus Angebot erstellen</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-5">
                  ...
                  <SelectContent>
                    {eligibleOffers.map((offer) => (
                      <SelectItem key={offer.id} value={offer.id}>
                        {offer.offer_number} · {offer.subject} · {formatEur(offer.gross_amount)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div>
                <Label htmlFor="invoice-due-days">Zahlungsziel in Tagen</Label>
                <Input
                  id="invoice-due-days"
                  type="number"
                  min="1"
              ...
              {selectedOffer && (
                <div className="rounded-xl bg-slate-50 p-4 text-sm">
                  <p className="font-semibold">{selectedOffer.subject}</p>
                  <p className="mt-1 text-muted-foreground">
                    {customerName(customersById.get(selectedOffer.customer_id))}
                  </p>
                  <p className="mt-3 text-lg font-bold">{formatEur(selectedOffer.gross_amount)}</p>
                  <p className="text-xs text-muted-foreground">
                    Positionen und Beträge werden vollständig übernommen.
                  </p>
                </div>
              )}
            </div>
            <DialogFooter>
              ...
              <Button
                ...
                className="bg-brand text-white hover:bg-brand/90"
              >
                {saving ? "Erstellt …" : "Entwurf erstellen"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
