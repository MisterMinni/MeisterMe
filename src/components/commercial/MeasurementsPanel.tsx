...
export function MeasurementsPanel({
  ...
  const visibleMeasurements = fixedSiteId
    ? data.measurements.filter((measurement) => measurement.site_id === fixedSiteId)
    : fixedCustomerId
      ? data.measurements.filter((measurement) => measurement.customer_id === fixedCustomerId)
      : data.measurements;
  ...
  return (
    <section className="rounded-2xl border border-border bg-card shadow-card">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4 sm:p-5">
        <div>
          <h2 className="font-display text-lg font-bold">Aufmaße</h2>
          <p className="text-sm text-muted-foreground">
            Mengen nachvollziehbar erfassen und für Angebote vorbereiten.
          </p>
        </div>
        {canWrite && (
          <Button
        ...
        {visibleMeasurements.length ? (
          <div className="space-y-3">
            {visibleMeasurements.map((measurement) => {
              const totals =
                measurement.totals &&
                typeof measurement.totals === "object" &&
                !Array.isArray(measurement.totals)
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate font-semibold">{measurement.title}</h3>
                    <p className="text-xs text-muted-foreground">
                      {customerName(customersById.get(measurement.customer_id ?? ""))} ·{" "}
                      {formatDate(measurement.captured_at)}
                    </p>
                  </div>
                  <div className="text-sm font-semibold">
                    {Number(totals.quantity ?? 0).toLocaleString("de-DE", {
                  ...
                  {canWrite ? (
                    <Select
                      ...
                      onValueChange={(status) => updateStatus(measurement.id, status)}
                    >
                      <SelectTrigger className="w-full md:w-40">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {Object.entries(MEASUREMENT_STATUS).map(([value, label]) => (
                    ...
                    <Badge variant="secondary">
                      {MEASUREMENT_STATUS[measurement.status] ?? measurement.status}
                    </Badge>
                  )}
                </article>
              );
          ...
          <div className="grid min-h-56 place-items-center rounded-xl border border-dashed border-border text-center">
            <div>
              <Ruler className="mx-auto h-8 w-8 text-slate-300" />
              <p className="mt-3 font-medium">Noch kein Aufmaß</p>
              <p className="text-sm text-muted-foreground">
                {data.customers.length
                  ? "Erfasse die erste Fläche direkt auf der Baustelle."
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <form onSubmit={saveMeasurement}>
            <DialogHeader>
              <DialogTitle>Aufmaß erfassen</DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 py-5 sm:grid-cols-2">
              ...
              {!fixedSiteId && (
                <div className="sm:col-span-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="none">Allgemeines Aufmaß ohne Baustelle</SelectItem>
              ...
              <div className="sm:col-span-2">
                <Label htmlFor="measurement-title">Bezeichnung</Label>
                <Input
                  id="measurement-title"
                  value={form.title}
                  onChange={(event) => setForm({ ...form, title: event.target.value })}
                  placeholder="z. B. Erdgeschoss – Wände"
                  required
                />
              </div>
              <div>
                <Label htmlFor="measurement-area">Raum / Bereich</Label>
                <Input
                  id="measurement-area"
                  value={form.area}
              ...
              <div>
                <Label htmlFor="measurement-description">Leistung / Bauteil</Label>
                <Input
                  id="measurement-description"
                  value={form.description}
                  onChange={(event) => setForm({ ...form, description: event.target.value })}
                  placeholder="Wandfläche streichen"
                  required
                />
              </div>
              <div>
                <Label htmlFor="measurement-length">Länge (m)</Label>
                <Input
              ...
              <div>
                <Label htmlFor="measurement-height">Höhe (m)</Label>
                <Input
              ...
              <div>
                <Label htmlFor="measurement-width">Breite (m)</Label>
                <Input
              ...
              <div>
                <Label htmlFor="measurement-deduction">Abzug (m²)</Label>
                <Input
                  id="measurement-deduction"
                  type="number"
                  min="0"
                  step="0.001"
                  value={form.deduction}
                  onChange={(event) => setForm({ ...form, deduction: event.target.value })}
                />
              </div>
              <div>
                <Label htmlFor="measurement-quantity">Menge manuell</Label>
                <Input
                  ...
                  placeholder={calculatedQuantity ? calculatedQuantity.toFixed(3) : "automatisch"}
                />
              </div>
              <div>
                <Label>Einheit</Label>
              ...
              <div className="sm:col-span-2">
                <p className="rounded-lg bg-brand/5 px-3 py-2 text-sm font-medium text-brand">
                  Ermittelte Menge:{" "}
                  {calculatedQuantity.toLocaleString("de-DE", { maximumFractionDigits: 3 })}{" "}
                  {form.unit}
                </p>
              </div>
              <div className="sm:col-span-2">
                <Label htmlFor="measurement-notes">Notizen</Label>
                <Textarea
            ...
            <DialogFooter>
              ...
              <Button
                type="submit"
                disabled={saving || (!fixedSiteId && !form.customerId)}
                className="bg-brand text-white hover:bg-brand/90"
              >
