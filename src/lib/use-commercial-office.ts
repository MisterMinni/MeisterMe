...
export function useCommercialOffice() {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<CommercialData> => {
      const [customers, measurements, materials, offers, invoices, communications, sites, timeEntries] = await Promise.all([
      ...
      const error = [customers, measurements, materials, offers, invoices, communications, sites, timeEntries].find(
        (result) => result.error,
      )?.error;
